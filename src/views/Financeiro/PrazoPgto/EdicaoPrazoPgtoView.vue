<template>
  <v-layout>
    ...
    <v-main>
      <div class="container-editar-prazopgto">
        <h2>Aqui você poderá editar novos prazos de pagamento!</h2>
        <form class="form-editar-prazopgto" @submit.prevent="editar">
          <v-row>
            <v-col cols="6">
              <v-text-field
                type="text"
...
export default {
  ...
  mounted() {
    ...
    axios
      .get(
        `${useEndpoints().getConsultaPrazoPgto}${useEndpoints().getEmpresaToken}/${
      ...
      .then((res) => {
        this.prazopgto = {
          prazopgto_id: res.data.prazopgto.prazopgto_id,
          empresa_id: res.data.prazopgto.empresa_id,
          prazopgto_nome: res.data.prazopgto.prazopgto_nome,
          prazopgto_tipo: this.prazopgto_tipos.find(
            (t) => t.nome === res.data.prazopgto.prazopgto_tipo
          ).valor,
          prazopgto_tipoforma: res.data.prazopgto.prazopgto_tipoforma.split(',')
        }
        if (this.prazopgto.prazopgto_tipo != 'V') {
          axios
            .get(
              `${useEndpoints().getConsultaPrazoPgtoDias}${useEndpoints().getEmpresaToken}/${
                this.prazopgto.prazopgto_id
              }`,
              {
                headers: {
                  Accept: 'application/json',
            ...
            .then((resParcela) => {
              resParcela.data.parcelas.forEach((p) => {
                this.prazos.push(p.dias)
              })
            })
            .catch((errParcela) => {
              console.log(errParcela)
            })
          this.loading = false
        } else {
          this.loading = false
                ...
                .catch((prazopgtodias_err) => {
                  if (
                    ...
                    this.setNotificacoes(
                      `Entre em contato com o suporte editarPrazoPgtoDias`,
                      'Erro',
                      'erro'
                    )
                  }
            ...
            if (err.response.data.errors) {
              ...
              for (const [chave, valor] of erros) {
                switch (chave) {
                  ...
                  default:
                    this.setNotificacoes(
                      'Entre em contato com o suporte editarprazopgtoView|editarprazopgto',
                      'Erro interno',
                      'erro'
                    )
                }
...
<style scoped>
.container-editar-prazopgto {
  padding: 4rem;
  font-family: 'Poppins', sans-serif;
  width: 90%;
}

.form-editar-prazopgto {
  padding: 3rem;
}
